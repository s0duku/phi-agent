...
use crate::terminal::PhiJobHandle;

const ENDPOINT_PREFIX: &str = "phi-job-";
const MAX_FRAME_SIZE: usize = 8 * 1024 * 1024;


fn validate_handle(handle: &str) -> io::Result<()> {
    if PhiJobHandle::is_valid(handle) {
        Ok(())
    } else {
        Err(io::Error::new(
...
pub(crate) fn bind(handle: &PhiJobHandle) -> io::Result<Listener> {
    let handle_str = handle.as_str_ref();
    validate_handle(handle_str)?;
    if !GenericNamespaced::is_supported()
        && let Some(parent) = endpoint_path(&format!("{ENDPOINT_PREFIX}{handle_str}")).parent()
    {
        std::fs::create_dir_all(parent)?;
    }
    let name = endpoint_name(handle_str)?;
    ListenerOptions::new()
        .name(name)
...
pub(crate) fn connect(handle: &PhiJobHandle) -> io::Result<Stream> {
    validate_handle(handle.as_str_ref())?;
    Stream::connect(endpoint_name(handle.as_str_ref())?)
}
